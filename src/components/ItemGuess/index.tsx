...
import optionList from "../../../options.json"
...
const ItemGuess = ({ 
  ...
  const filtered = optionList.filter(x => x.name.toLowerCase().includes(input.toLowerCase()))
  ...
  useEffect(() => {
    setClick(false)
    if(input.toLowerCase() !== gameName.toLowerCase()) return
  ...
  return (
    <Item color={guessedColor} correct={guessedBy || correct}>
      ...
      <InputWrapper>
        <Input disabled={forfeit || guessedBy} value={(forfeit || guessedBy) ? gameName : input} style={forfeit ? { backgroundColor: "#f23f3f", color: "#fff" } : {}} onChange={({ target: { value } }) => setInput(value)} type="text" />
      </InputWrapper>
      {
        input.length >= 3 && filtered.length && !correct && !clicked ? (
          <Select>
            {filtered.map(({ name, ids }, index) => (<Option onClick={() => checkResult(name, ids)} key={index} value={name}>{name}</Option>))}
          </Select>
        ) : null
      }
